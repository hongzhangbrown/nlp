...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(name = 'input',shape = [None,self.config.window_size],dtype = tf.int32)
    self.labels_placeholder = tf.placeholder(name = 'labels',shape = [None,self.config.label_size],dtype = tf.float32)
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
        with tf.variable_scope("embedding_layer") as scope:
            print len(self.wv),self.config.embed_size
            embedding = tf.get_variable(name = "embedding", shape = [len(self.wv),self.config.embed_size], initializer = xavier_weight_init())
  ...
  def add_model(self, window):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope('layer',initializer = xavier_weight_init()) as _scope:
        W = tf.get_variable('W_l',[self.config.window_size*self.config.embed_size,self.config.hidden_size])
        b1 = tf.get_variable('b1',
                          shape = [self.config.hidden_size])
        variable_summaries(W,W.name)
        variable_summaries(b1,b1.name)
    with tf.variable_scope('softmax',initializer = xavier_weight_init()) as softmax_layer:
        U = tf.get_variable('U_h',shape = [self.config.hidden_size,self.config.label_size])
  ...
  def add_training_op(self, loss):
    ...
    train_op = tf.train.AdamOptimizer(self.config.lr).minimize(loss,global_step = global_step)
