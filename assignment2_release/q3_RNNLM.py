...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(shape = [None,self.config.num_steps],dtype = tf.int32)
    self.labels_placeholder = tf.placeholder(shape = [None,self.config.num_steps],dtype = tf.float32)
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding_layer") as scope:
        embedding = tf.get_variable("embedding", [len(self.vocab),self.config.embed_size], initializer = tf.random_uniform_initializer(-1,1),trainable = True)
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("projection_layer") as scope:
      U = tf.get_variable('U', [self.config.hidden_size,len(self.vocab)], initializer= tf.random_uniform_initializer())
    
      b_2 = tf.get_variable('b2',[len(self.vocab)], initializer = tf.constant_initializer(0.0))
      variable_summaries(U,U.name)
      variable_summaries(b_2,b_2.name)
    outputs = []
    for rnn in rnn_outputs:
       out = tf.matmul(rnn,U) + b_2
       outputs.append(out) 
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('rnn') as scope:
      ...
      for step,current_input in enumerate(inputs):
        ...
        Whh = tf.get_variable('Whh',[self.config.hidden_size,self.config.hidden_size], initializer = tf.random_normal_initializer(0,1.))
        Whx = tf.get_variable('Whx',[self.config.embed_size,self.config.hidden_size], initializer = tf.random_normal_initializer(0,1.))
        b_1 = tf.get_variable('b_h',[self.config.hidden_size],initializer = tf.constant_initializer(0.))
    ...
    with tf.variable_scope('RNNdropout') as scope:
